<script>
  ...
  async function fetchPlainText(canvas) {
  ...
  function processPlainText(ocrText) {
    if ( ! ocrText ) { return ''; }
  ...
  $: if ( loaded && dragonEl !== undefined && dragon === undefined ) {
    ...
    console.log("AHOY", loaded, dragonEl, getInfoUrl(canvases[canvasIdx - 1]));
...
<PaneGroup direction="horizontal">
  <!-- <PaneResizer class="pane--resizer {headerTabs.pages && headerTabs.image ? '' : 'hidden'}">
    <button class="pane--resizer--thumb">
      <span class="material-icons">drag_indicator</span>
    </button>
  </PaneResizer> -->
  <Pane defaultSize={80} class="order-2">
    {#if loaded}
      <div style="display: grid; grid-template-rows: minmax(0, 1fr) min-content; height: 100%;">
        <PaneGroup direction="horizontal" defaultSize={75} class="position-relative {inPageTransition ? 'invisible' : ''}">
          <Pane defaultSize={60} class="flex {headerTabs.image ? '' : 'hidden'}">
          ...
          <Pane defaultSize={40} class="{(( !plainText && headerTabs.image ) || !headerTabs.plaintext) ? 'hidden' : ''}">
  ...
  <Pane defaultSize={20} maxSize={50} class="viewer--sidebar order-1 {headerTabs.pages ? '' : 'hidden'}">
